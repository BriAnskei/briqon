...
const BREAK_FREQUENCY_INSTRUCTIONS: Record<Exclude<BreakFrequency, null>, string> = {
  "few-long":
...
export class WizardPromptBuilder {
  ...
  public static build(state: NewScheduleFormState): BuiltPrompt {
    if (state.scheduleType === "personal") {
      return WizardPromptBuilder.buildPersonalPrompt(state);
    }
    if (state.scheduleType === "event") {
      return WizardPromptBuilder.buildEventPrompt(state);
    }
  ...
  private static buildPersonalPrompt(state: NewScheduleFormState): BuiltPrompt {
    ...
    if (focusText) {
      ...
      if (state.priorityDurationMinutes != null && state.priorityDurationMinutes > 0) {
        lines.push(
    ...
    if (state.appointments.length > 0) {
      ...
      for (const appt of state.appointments) {
        ...
        lines.push(`  • ${appointmentLabel(appt)}: ${start} – ${end} (${dur} min)`);
      }
    }
