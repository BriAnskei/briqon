...
export class ModelManager {
  static async ensureModel(onProgress?: (percent: number) => void): Promise<string> {
    const exists = await ModelManager.exists();

    if (!exists) {
      return ModelManager.download(onProgress);
    }

    return ModelManager.getPath();
  }
  ...
  static async download(onProgress?: (percent: number) => void): Promise<string> {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        try {
          ...
          console.warn("Failed to prepare resume data, restarting download", resumeError);
