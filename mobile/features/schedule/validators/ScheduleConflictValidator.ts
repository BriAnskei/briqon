import { MealPlacement, type NewScheduleFormState } from "@/type/NewScheduleTypes";
...
export default class ScheduleConflictValidator {
  ...
  public validateTimeBlocksWithinWindow(): ValidatorResType {
    ...
    for (const block of blocks) {
      const start = normalizeMinute(getMinutesOfDay(block.start), windowStart);

      let end = normalizeMinute(getMinutesOfDay(block.end), windowStart);
  ...
  private getTimeBlocks(): TimeBlock[] {
    const appointments: TimeBlock[] = this.form.appointments.map((app) => ({
      id: app.id,
      label: app.type === "custom" ? app.customLabel || "Custom appointment" : app.type,
      start: app.startTime,
      end: app.endTime,
    }));
    ...
    const fixedMeals: TimeBlock[] = this.form.meals
      ...
      .map((meal) => ({
        ...
        end: new Date(meal.fixedTime!.getTime() + meal.durationMinutes * 60 * 1000),
      }));
