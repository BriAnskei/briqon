...
const useEventItems = ({ form, setForm }: Payload): UseEventItemsStateType => {
  ...
  const commitEventItem = () => {
    if (
      ...
      (!eventItemDraft.durationHours.trim() && !eventItemDraft.durationMinutes.trim())
    )
      return;
    ...
    const item: EventScheduleItem = {
      ...
      fixedTime: eventItemDraft.isFixedTime ? eventItemDraft.fixedTime : undefined,
    };
  ...
  const toggleFixedTime = () => {
    ...
    patchEventItem({
      isFixedTime: next,
      fixedTime: next ? (eventItemDraft.fixedTime ?? defaultFixedTime()) : undefined,
  ...
  return {
    eventItemDraft,
    patchEventItem,
    showDraft: () => setEventItemDraft({ ...defaultEventItemDraft(), visible: true }),
