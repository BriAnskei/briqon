 ...
 */
export function useWizardValidation({ form, step, isEvent }: UseWizardValidationParams) {
  const validator = useMemo(() => new ScheduleFormWindowtimeRuleValidator(form), [form]);

  const conflictValidator = useMemo(() => new ScheduleConflictValidator(form), [form]);

  const eventValidator = useMemo(() => new EventScheduleValidator(form), [form]);
  ...
  const stepError = useMemo(() => {
    ...
    if (isEvent) {
      ...
      if (step === 3) {
        const errors = [
          !validation.eventItemsPresent.valid && validation.eventItemsPresent.message,
      ...
      if (step === 1) {
        ...
        return !validation.windowTime.valid ? validation.windowTime.message : undefined;
      }
