...
import { STEP_LABELS, STEP_ORDER, type Step } from "../constants";
  ...
  return (
    <View style={s.progressCard}>
      {STEP_ORDER.map((key) => {
        ...
        const active = !done && completedSteps.length === STEP_ORDER.indexOf(key);
        return (
          <View key={key} style={s.stepRow}>
            ...
            <Text style={[s.stepLabel, done && s.stepLabelDone]}>{STEP_LABELS[key]}</Text>
            {active && (
              <ActivityIndicator
                size="small"
