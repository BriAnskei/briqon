...
export function useWizardForm() {
  ...
  const personalSummary = useMemo<PersonalSummary | null>(() => {
    ...
    const breakMinutes = form.breakFrequency ? validator.getBreakWindowMin() : 0;
  ...
  const showStepErrorToast = useCallback((message: string) => {
    Toast.show({
      type: "error",
      text1: "Invalid Input",
      text2: message,
  ...
  }, [stepError, showStepErrorToast, step, form]);
