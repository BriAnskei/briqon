...
import type { BreakFrequency, NewScheduleFormState } from "@/type/NewScheduleTypes";
...
export default class ScheduleFormWindowtimeRuleValidator {
  ...
  public validatePriorityTimeWindow(): ValidatorResType {
    const totalWindowTime = this.getWindowMinutes();
    const overAllWindowEvent =
      this.getAppWindowMin() + this.getMealsWindowMin() + this.getBreakWindowMin();

    const remainingWindowMin = totalWindowTime - overAllWindowEvent;
  ...
  private findApplicableBreakFreq(totalFixedEventSchedMins: number): NonNullable<BreakFrequency> {
    ...
    for (const [breakStyle, percentage] of Object.entries(breakTypePercentage)) {
      if (totalFixedEventSchedMins + totalTimeWindow * percentage < totalTimeWindow) {
  ...
  public validateMealWindowTime(): ValidatorResType {
    ...
    const fixedSched = this.getMealsWindowMin() + this.getAppointmentsTotalMinutes();
  ...
  public validateAppWindowTime(): ValidatorResType {
    ...
    if (fixedSched > windowMin) {
      return {
        valid: false,
        message: "The total appointment duration exceeds the schedule time window.",
      };
    }
  ...
  public getBreakWindowMin() {
    const breakFreqPercentage =
      this.getBreakPercentage()[this.form.breakFrequency as NonNullable<BreakFrequency>];
  ...
  private getTotalWindowMin(schedules: { start_time: Date; end_time: Date }[]) {
    let res = 0;
    for (const { start_time, end_time } of schedules) {
