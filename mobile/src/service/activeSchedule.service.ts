...
import { type ActiveSchedule, CreateActivecheduleSchema } from "../models/activeSchedule.model";
...
export class ActiveScheduleService {
  ...
  async createAsync(payload: { activationPayload: CreationPayload; overWrite: boolean }) {
    const { activationPayload, overWrite } = payload;

    const { recurringConflict, nonRecurringConflict } =
      await this.checkScheduleActivationConflicts(activationPayload);

    if (!overWrite && (recurringConflict.length > 0 || nonRecurringConflict.length > 0)) {
      const overAllConflictArr = [...recurringConflict, ...nonRecurringConflict];
    ...
    await this.resolveActivationConflicts({
      ...
      isNewActiveRecurring: activationPayload.newActiveSchedule.recurring,
      newActiveScheduleDays: activationPayload.selectedDays,
    });

    const activeScheduleId = ulid();
    this.repo.transaction(async (db) => {
      if (activationPayload.isScheduleNeedsToSave && activationPayload.newSchedule) {
        await this.scheduleRepo.create(activationPayload.newSchedule, db);
      }

      this.repo.create({ ...activationPayload.newActiveSchedule, id: activeScheduleId }, db);

      if (activationPayload.newActiveSchedule.active_type === "days") {
        const activeDays: ActiveScheduleDays[] = (activationPayload.selectedDays ?? []).map(
          (d) => ({
            id: ulid(),
      ...
      } else if (activationPayload.newActiveSchedule.active_type === "date") {
        const selectedDate = activationPayload.selectedDate;

        if (selectedDate === undefined) throw new Error("No data for the date selection");
  ...
  private async checkScheduleActivationConflicts(payload: CreationPayload): Promise<{
  ...
  */
  private async resolveActivationConflicts(payload: {
    ...
    const { recurringConflict, nonRecurringConflict, isNewActiveRecurring, newActiveScheduleDays } =
      payload;
  ...
  private async handleActiveTypePersistence(payload: {
    ...
    const { dayOfWeeks, date, activeScheduleId, activeScheduleType, db } = payload;

    if (activeScheduleType === "date") {
      await this.createDateOfActiveSchedule(date!, activeScheduleId, db);
    } else if (activeScheduleType === "days") {
      await this.createSelectedDaysOfActiveSchedule(dayOfWeeks!, activeScheduleId, db);
    }
  }
