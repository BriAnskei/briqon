...
import type { ItemUIState } from "../types/EditScheduleType";
...
export const useEditSchedule = () => {
  ...
  const setItemState = useCallback((index: number, next: ItemUIState) => {
    setItemStates((prev) => {
      ...
      Object.entries(prev).forEach(([k, v]) => {
        const i = Number(k);
        const shouldCollapseOtherEditing = i !== index && v.status === "editing";
  ...
  const handleDone = useCallback(async () => {
    ...
    Object.entries(itemStates).forEach(([k, v]) => {
      const i = Number(k);
      if (v.status === "queued_edit") edits.push({ itemIndex: i, prompt: v.prompt });
